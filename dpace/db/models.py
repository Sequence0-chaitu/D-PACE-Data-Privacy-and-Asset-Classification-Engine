...
def clear_asset_records(conn: sqlite3.Connection, asset_id: int) -> None:
    """
    Remove previous scan results and violations for *asset_id*.

    Called before re-inserting so that repeated scans of the same tree stay
    idempotent: the most recent scan is the single source of truth, rather
    than each run stacking duplicate rows onto the dashboard totals.
    """
    conn.execute("DELETE FROM scan_results WHERE asset_id = ?", (asset_id,))
    conn.execute("DELETE FROM policy_violations WHERE asset_id = ?", (asset_id,))
    conn.commit()
